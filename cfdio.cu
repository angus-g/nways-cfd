...
void writedatafiles(double *psi, int m, int n, int scale) {
  ...
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++) {
      vel[i][j][0] =
          (psi[(i + 1) * (m + 2) + j + 2] - psi[(i + 1) * (m + 2) + j]) / 2.0;
      vel[i][j][1] =
          -(psi[(i + 2) * (m + 2) + j + 1] - psi[i * (m + 2) + j + 1]) / 2.0;
...
void writeplotfile(int m, int n, int scale) {
  ...
  fprintf(gnuplot,
          ...
          "1:2:(%d*0.75*$3/sqrt($3**2+$4**2)):(%d*0.75*$4/sqrt($3**2+$4**2)) "
          "with vectors  lc rgb \"#7F7F7F\"",
          scale, scale);
